...
- (id)initWithSize:(NSSize)size {
	if (self = [super init]) {
		ctx = [ANImageBitmapRep CreateARGBBitmapContextWithSize:*(CGSize *)&size];
...
- (ANImageBitmapRep *)cropWithFrame:(CGRect)r {
	...
	CGRect ibounds = CGRectMake(0, 0, [self size].width, [self size].height);
	ANImageBitmapRep * retI = [[ANImageBitmapRep alloc] initWithSize:*(NSSize *)&r.size];
	CGContextRef _ctx = [retI graphicsContext];
	CGContextBeginPath(_ctx);
	CGContextAddRect(_ctx, rbounds);
	CGContextClosePath(_ctx);
	CGContextSaveGState(_ctx);
	CGContextClip(_ctx);
	ibounds.origin.x -= r.origin.x;
	ibounds.origin.y -= r.origin.y;
	
	
	CGContextTranslateCTM(_ctx, 0, [self size].height);
	CGContextScaleCTM(_ctx, 1.0, -1.0);
	CGContextDrawImage(_ctx, ibounds, [self CGImage]);
